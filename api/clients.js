...
export default async function handler(req, res) {
    try {
        ...
        // Rota para adicionar um novo cliente (POST) - ATUALIZADA
            ...
            const query = `
                INSERT INTO clients (name, "startDate", cpf, phone, "loanValue", "dailyValue", "paymentDates", installments, frequency, files) 
        ...
        // Rota para atualizar um cliente (PUT) - ATUALIZADA
            ...
            const { id, name, startDate, cpf, phone, loanValue, dailyValue, paymentDates, installments, frequency, files } = req.body;
            if (!id) return res.status(400).json({ error: 'Client ID is required' });

            const query = `
                UPDATE clients 
                SET name = $1, "startDate" = $2, cpf = $3, phone = $4, "loanValue" = $5, "dailyValue" = $6, "paymentDates" = $7, installments = $8, frequency = $9, files = $10
                WHERE id = $11
                RETURNING *
            `;
            const values = [name, startDate, cpf, phone, loanValue, dailyValue, JSON.stringify(paymentDates), installments, frequency, JSON.stringify(files || []), id];
            const result = await db.query(query, values);
